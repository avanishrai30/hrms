...
async function main(): Promise<void> {
  ...
  const bootstrapPasswordHash = await argon2.hash(bootstrapPassword);
  await prisma.platformUser.upsert({
    ...
    update: { passwordHash: bootstrapPasswordHash, role: "PLATFORM_SUPER_ADMIN", status: "ACTIVE" }
  });
  ...
  const tenantUserPasswordHash = await argon2.hash(bootstrapPassword);
  const userOwner = await prisma.user.upsert({
    ...
    update: { passwordHash: tenantUserPasswordHash, status: "ACTIVE" }
  });
