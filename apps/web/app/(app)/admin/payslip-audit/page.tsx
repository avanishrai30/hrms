...
export default function AdminPayslipAuditPage() {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      setError(null);
      const data = await apiRequest<AuditEntry[]>("/payslips/audit?limit=100");
      setLogs(data ?? []);
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Failed to load payslip audit trail.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getActionBadge = (action: string) => {
    switch (action) {
      ...
      case "payslip.regenerated":
        return <Badge tone="warning">Regenerated (v+1)</Badge>;
      case "payslip.distributed":
        return <Badge tone="neutral">Batch Distributed</Badge>;
      case "payslip.email.sent":
        return <Badge tone="success">Email Sent</Badge>;
      case "payslip.email.failed":
        return <Badge tone="danger">Email Failed</Badge>;
      case "payslip.downloaded":
        return <Badge tone="success">PDF Downloaded</Badge>;
      case "payslip.viewed":
        return <Badge tone="neutral">Viewed</Badge>;
      case "payslip.archived":
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">
            Payslip Audit Trail Center
          </h1>
          <p className="text-sm text-slate-500">
            Immutable audit record of all PDF creations, downloads, employee views, and distribution events.
          </p>
        </div>
      ...
      <div className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <h2 className="text-base font-bold text-slate-900">Audited Payslip Operations</h2>
          <span className="text-xs text-slate-500">{logs.length} recorded events</span>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="p-12 text-center text-sm text-slate-500">
            No payslip audit events logged yet.
          </div>
        ) : (
