    ...
    return (
      <div className="p-8 max-w-lg mx-auto text-center mt-12">
        <div className="p-8 rounded-card bg-surface-raised border border-border-subtle shadow-card space-y-3">
          ...
          <h2 className="text-base font-bold text-foreground">Teams Access Restricted</h2>
          <p className="text-xs text-foreground-muted">
            You do not have permission (`organization.view`) to access organizational teams.
          </p>
        </div>
      </div>
    );
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto space-y-6 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">Workplace Teams</h1>
          <p className="text-xs text-foreground-muted">
            Functional cross-departmental and departmental working groups.
          </p>
        </div>
      ...
      {isError ? (
        <div className="p-8 rounded-card bg-surface-raised border border-border-subtle text-center space-y-3">
          ...
          <p className="text-xs font-semibold text-foreground">Teams unavailable</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {teams.map((team) => (
            <div
              key={team.id}
              className="rounded-card bg-surface-raised border border-border-subtle p-5 shadow-card hover:border-primary/30 transition flex flex-col justify-between space-y-2"
            >
              <div>
                <div className="flex items-center justify-between">
                  <h3 className="text-xs font-bold text-foreground">{team.name}</h3>
                ...
                {team.department && (
                  <p className="text-[11px] text-foreground-muted font-medium mt-0.5">
                    {typeof team.department === "string" ? team.department : (team.department as { name?: string })?.name || "—"}
                  </p>
                )}
        ...
        <div className="py-16 text-center rounded-card bg-surface-raised border border-border-subtle flex flex-col items-center justify-center text-foreground-muted">
          ...
          <p className="text-[11px] text-foreground-muted mt-0.5">Add working teams to organize project squads.</p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-md rounded-panel bg-surface-raised border border-border-subtle p-6 shadow-2xl space-y-4 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between pb-3 border-b border-border-subtle">
              <h3 className="text-sm font-bold text-foreground">Add Workplace Team</h3>
              <button
            ...
            <form onSubmit={handleCreate} className="space-y-4 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-bold text-foreground">Team Name *</label>
                <input
                  type="text"
                  value={name}
              ...
              <div className="space-y-1">
                <label className="font-bold text-foreground">Team Code *</label>
                <input
                  type="text"
                  value={code}
                  onChange={(e) => setCode(e.target.value)}
                  placeholder="e.g. SQUAD-CORE"
                  className="w-full px-3 py-2 rounded-control bg-surface border border-border text-foreground focus:outline-none focus:ring-2 focus:ring-primary/20"
                  required
                />
              </div>
              ...
              {departments.length > 0 && (
                <div className="space-y-1">
                  <label className="font-bold text-foreground">Associated Department</label>
                  <select
                    value={departmentId}
